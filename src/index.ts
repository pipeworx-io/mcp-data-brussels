 ...
 *
 * Brussels Open Data — open data for the City of Brussels, Belgium via the OpenDataSoft Explore v2.1 API: city services, mobility, culture & urban data. Keyless.
 * Use `search_datasets` (find a dataset_id) -> `dataset_info` (schema) -> `query` (ODSQL records).
 */


const BASE = 'https://opendata.bruxelles.be';
const UA = 'pipeworx-mcp-data-brussels/1.0 (+https://pipeworx.io)';

const tools: McpToolExport['tools'] = [
  {
    name: 'search_datasets',
    description: 'Search Brussels Open Data for datasets by keyword (city services, mobility, culture & urban data). Returns dataset_ids (pass to query/dataset_info), titles, themes and record counts.',
  ...
  {
    name: 'dataset_info',
    description: 'Get metadata for one Brussels Open Data dataset (fields/schema, themes, record count) — call before query to learn the column names.',
    inputSchema: { type: 'object', properties: {
  ...
  {
    name: 'query',
    description: 'Query records from a Brussels Open Data dataset with ODSQL. Filter (where), aggregate (group_by/select), sort (order_by), paginate (limit/offset).',
    inputSchema: { type: 'object', properties: {
      dataset_id: { type: 'string', description: 'Dataset id from search_datasets.' },
...
async function get(path: string): Promise<unknown> {
  ...
  if (!res.ok) throw new Error(`Brussels Open Data: ${res.status} ${await res.text().then((t) => t.slice(0, 160))}`);
  return res.json();
}
